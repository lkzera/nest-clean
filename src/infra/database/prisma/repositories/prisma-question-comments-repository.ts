...
import { PrismaCommentWithAuthMapper } from '../mappers/prisma-comment-with-author-mapper';
...
export class PrismaQuestionCommentsRepository implements QuestionCommentsRepository {
  ...
  async findById(id: string): Promise<QuestionComment | null> {
    const questionComment = await this._prisma.comment.findUnique({
      where: {
        id
      }
    });
  ...
  async findManyByQuestionId(questionId: string, { page }: PaginationParams): Promise<QuestionComment[]> {
    const questionComments = await this._prisma.comment.findMany({
      where: {
        questionId
      },
      orderBy: {
  ...
  async findManyByQuestionIdWithAuthor(questionId: string, { page }: PaginationParams): Promise<CommentWithAuthor[]> {
    ...
    return questionComments.map(PrismaCommentWithAuthMapper.toDomain);
  }

  async create(questionComment: QuestionComment): Promise<void> {
    const data = PrismaQuestionCommentMapper.toPrisma(questionComment);

    await this._prisma.comment.create({ data });
  }
